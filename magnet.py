...
class Conv2D_BN_activa(nn.Module):
    def __init__(
            ...
            padding=0, dilation=1, if_bn=False, activation='relu', bias=None, initializer=None
    ):
...
class ResBlk(nn.Module):
    def __init__(self, dim_in, dim_out, dim_intermediate=32, ks=3, s=1):
        ...
        p = (ks - 1) // 2
        self.cba_1 = Conv2D_BN_activa(
            dim_in, dim_intermediate, ks, s, p,
            if_bn=False, activation='relu', bias=False
        )
        self.cba_2 = Conv2D_BN_activa(
            dim_intermediate, dim_out, ks, s, p,
            if_bn=False, activation=None, bias=False
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        if self.use_texture_conv:
            self.texture_cba = Conv2D_BN_activa(
        ...
        if self.use_motion_conv:
            self.motion_cba = Conv2D_BN_activa(
                32, 32, 3, 1, 1,
                if_bn=False, activation='relu', bias=False
            )
...
class Encoder_D(nn.Module):
    def __init__(
            self, dim_in=3, dim_out=32, num_resblk=3,
            use_texture_conv=True, texture_downsample=True,
            num_resblk_texture=2
    ):
        super(Encoder_D, self).__init__()
        self.use_texture_conv = use_texture_conv
...
class Decoder(nn.Module):
    ...
    def forward(self, texture, motion):
        if self.texture_downsample:
            texture = self.texture_cba(self.texture_up(texture))
...
class Manipulator(nn.Module):
    def __init__(self):
        ...
        self.g = Conv2D_BN_activa(32, 32, 3, 1, 1)
        self.h_conv = Conv2D_BN_activa(32, 32, 3, 1, 1, activation=None)
        self.h_resblk = ResBlk(32, 32)

    def forward(self, motion_A, motion_C, amp_factor):
        diff = motion_C - motion_A
        diff = self.g(diff)
        diff = self.h_conv(diff)
        diff = self.h_resblk(diff)
        motion_mag = motion_A + diff
        return motion_mag
...
class MagNet(nn.Module):
    ...
    def forward(self, batch_A, batch_B, batch_C, amp_factor=1, mode='train'):

        if mode == 'train':
            # Missing V'Y in the original paper.
            texture_A, motion_A = self.encoder(batch_A)
            texture_B, motion_B = self.encoder(batch_B)
            texture_C, motion_C = self.encoder(batch_C)

            motion_mag = self.manipulator(motion_A, motion_C, amp_factor)
            texture_AC = [texture_A, texture_C]
            mag_A = self.decoder(texture_A, motion_mag)

            motion_BC = [motion_B, motion_C]

            return mag_A, texture_AC, motion_BC
        elif mode == 'evaluate':
            texture_A, motion_A = self.encoder(batch_A)
            texture_C, motion_C = self.encoder(batch_C)
            motion_mag = self.manipulator(motion_A, motion_C, amp_factor)
